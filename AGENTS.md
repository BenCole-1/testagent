...
└── docs/
    ├── index.md                  # Documentation navigation and bucket index
    ├── notes/
    │   └── yc-20x-company-note.md    # Distilled external strategy note
    └── playbooks/
        └── agent-knowledge-ops.md    # Core knowledge-ops playbook
```

Key areas:
- `.git/`: Version control internals; do not modify manually.
- `docs/`: Primary documentation area organized into buckets (`playbooks/`, `notes/`) plus `index.md`.
...
| View doc content | `sed -n '1,240p' docs/playbooks/agent-knowledge-ops.md` | Replace path as needed. |
...
- **Canonical content**: Original files in `docs/` (especially `playbooks/` and `notes/`) that represent source guidance.
- **Index layer**: Derived metadata/search layer described in `docs/playbooks/agent-knowledge-ops.md`.
